...
# Spatial Relationship Mapping (SRM) Attention Module
class SRM(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, 1, kernel_size=3, padding=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels, 1, kernel_size=3, padding=1, bias=True)
        self.conv3 = nn.Conv2d(in_channels, 1, kernel_size=3, padding=1, bias=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # Extract spatial relationships from three different operations
        b1 = self.sigmoid(self.conv1(x))
        b2 = self.sigmoid(self.conv2(x))
        b3 = self.sigmoid(self.conv3(x))
        
        # Fuse spatial relationships
        fusion = b1 + b2 + b3
        fusion = fusion / 3.0
...
def train_model(model, train_loader, val_loader, num_epochs=200, learning_rate=2e-4, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['loss'] < best_val_loss - 1e-4:
            ...
            torch.save(model.state_dict(), 'best_unet_srm_mt.pth')
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('training_curves_unet_srm_mt.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Test evaluation
    print("Loading best model for test evaluation...")
    model.load_state_dict(torch.load('best_unet_srm_mt.pth', map_location=device))
    model.to(device)
    ...
    print("\n" + "="*50)
    print("U-NET + SRM TEST EVALUATION METRICS - MAGNETIC TILES")
    print("="*50)
    ...
    plot_confusion_matrix(test_metrics["confusion"], "Test Set Confusion Matrix (UNet+SRM MT)")
    
    print("U-Net with SRM Attention Magnetic Tiles training and evaluation completed!")
