...
# Squeeze and Excitation Module with r=12
...
class DoubleConvWithAttention(nn.Module):
    def __init__(self, in_ch, out_ch, dropout_rate=0.1, use_dropout=True):
        ...
        self.double_conv = nn.Sequential(*layers)
        self.se_block = SEBlock(out_ch, reduction=12)
...
def train_model(model, train_loader, val_loader, num_epochs=200, learning_rate=2e-4, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['loss'] < best_val_loss - 1e-4:
            ...
            torch.save(model.state_dict(), 'best_unet_se_mt.pth')
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('training_curves_unet_se_mt.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Test evaluation
    print("Loading best model for test evaluation...")
    model.load_state_dict(torch.load('best_unet_se_mt.pth', map_location=device))
    model.to(device)
    ...
    print("\n" + "="*50)
    print("U-NET + SE (r=12) TEST EVALUATION METRICS - MAGNETIC TILES")
    print("="*50)
    ...
    plot_confusion_matrix(test_metrics["confusion"], "Test Set Confusion Matrix (UNet+SE MT)")
    
    print("U-Net with SE Attention Magnetic Tiles training and evaluation completed!")
