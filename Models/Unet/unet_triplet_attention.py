...
class AttentionGate(nn.Module):
    def __init__(self, in_planes):
        ...
        self.conv = BasicConv(in_planes, 1, kernel_size=1, stride=1, padding=0, relu=False)
        self.spatial = ZPool()
...
class TripletAttention(nn.Module):
    def __init__(self, no_spatial=False):
        ...
        self.cw = AttentionGate(2)
        self.hc = AttentionGate(2)
        self.no_spatial=no_spatial
        if not no_spatial:
            self.hw = AttentionGate(2)
    def forward(self, x):
        ...
        if not self.no_spatial:
            x_out = self.hw(x)
            x_out = 1/3 * (x_out11 + x_out21 + x_out)
        else:
...
def train_model(model, train_loader, val_loader, num_epochs=200, learning_rate=2e-4, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['loss'] < best_val_loss - 1e-4:
            ...
            torch.save(model.state_dict(), 'best_unet_triplet_mt.pth')
...
def main():
    ...
    plt.tight_layout()
    plt.savefig('training_curves_unet_triplet_mt.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Test evaluation
    print("Loading best model for test evaluation...")
    model.load_state_dict(torch.load('best_unet_triplet_mt.pth', map_location=device))
    model.to(device)
    ...
    print("\n" + "="*50)
    print("U-NET + TRIPLET ATTENTION TEST EVALUATION METRICS - MAGNETIC TILES")
    print("="*50)
    ...
    plot_confusion_matrix(test_metrics["confusion"], "Test Set Confusion Matrix (UNet+Triplet MT)")
    
    print("U-Net with Triplet Attention Magnetic Tiles training and evaluation completed!")
